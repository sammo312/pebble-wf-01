...
function buildConfigPage(current) {
  var html = [
    ...
    "  var topChar=ramp.charAt(ramp.length-1);",
    "  var html='';",
    ...
    "      }else{",
    "        var dr=r-rows/2,dc=c-cols/2;",
    "        var n=Math.abs(Math.sin(dr*0.4+dc*0.5)+Math.cos(dr*0.3-dc*0.4))/2;",
    "        var idx=Math.floor(n*ramp.length);",
